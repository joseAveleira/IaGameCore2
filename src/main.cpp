...
const char *mqtt_server = "192.168.0.179"; // 192.168.0.178 0 192.168.1.229;
...
void setup()
{
  ...
  delay(2000);
  M5.Lcd.clearDisplay();
  M5.Lcd.drawXBitmap(0, 0, portada, portadaWidth, portadaHeight, TFT_GREEN, TFT_BLACK);
  //pantall en negro
...
void reConnect()
{
  while (!client.connected())
  {
    ...
    if (client.connect("NodeMCU_Client"))
    {
      ...
      M5.Lcd.printf("\nSuccess\n");
      client.subscribe("/IAGameResponse");
      delay(1000);
    }
    else
    {
...
void publisOption(char option)
{
  ...
  client.publish("/IAGame", message.c_str());
  SetVibration(60);
  M5.Lcd.clearDisplay();
